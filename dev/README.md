...
2. **Accelerated Outbound Climb:** Rapidly steps up through intermediate orbital gears ($300\times \rightarrow 600\times \rightarrow 1800\times \rightarrow 3600\times$) as Earth's gravity well recedes.
3. **Deep Space 7.2kx Cruise:** Cruises the vast cislunar abyss at **7,200x** ($1\text{ second} = 2\text{ hours}$) in a protected deep-space corridor.
...
7. **Graduated Earth Approach & Entry Interface:** Drops progressively through safety tiers into Earth's gravity well, locking to 1x at $122\text{ km}$ to endure a brutal $12.3\text{G}$ atmospheric deceleration and plasma blackout.
...
- **Manual Gear Shift:** Once Auto-Warp is **OFF**, the flight computer stops automatic speed adjustments. You have complete manual authority over all 9 gear levels ($1\times, 10\times, 30\times, 60\times, 300\times, 600\times, 1800\times, 3600\times, 7200\times$).
- **WebXR Motion Comfort:** All speed changes use our continuous **Cubic Hermite Smoothstep Gliding Protocol** ($1.0\text{s}$ ease-in/ease-out per tier) to eliminate sudden visual tearing and prevent motion sickness.
...
- **Running Hot with Auto-Warp ON:** You can manually click a **Yellow Advisory** speed (such as $300\times$ or $600\times$ during lunar approach). The flight computer registers your command intent (`userSelectedWarp`) and maintains the faster speed until the safety envelope drops!
...
* **Continuous Hermite Time-Warp Easing:** Engineered continuous $1.0\text{s}$ cubic smoothstep rate interpolation (`ease = 3p^2 - 2p^3`), eliminating all instantaneous speed chops for pristine 6DoF WebXR comfort.
* **Tighter Lunar Approach:** Retains $3.6\text{kx}$ down to $42,000\text{ km}$, $1.8\text{kx}$ down to $22,000\text{ km}$, and $600\times$ down to $12,000\text{ km}$, stepping down to $60\times$ at $\le 5,500\text{ km}$ for the perilune sizzle shot.
* **Earlier Outbound & Departure Ramps:** Promoted post-TLI climb ($1.8\text{kx}$ at $10\text{k km}$, $3.6\text{kx}$ at $22\text{k km}$, $7.2\text{kx}$ at $45\text{k km}$) and lunar departure ($300\times$ at $5\text{k km}$, $600\times$ at $10\text{k km}$, $1.8\text{kx}$ at $22\text{k km}$).
* **10x Parachute Descent Schedule:** Auto-Warp maintains $10\times$ Green during settled post-plasma glide and parachute descent, smoothly downshifting to $1\times$ for drogue deployment ($8.5 \rightarrow 7.3\text{ km}$), main deployment ($3.4 \rightarrow 3.0\text{ km}$), and touchdown ($\le 350\text{ m}$).
