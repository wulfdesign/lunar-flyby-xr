...
- `Tasks.md` -> Tasks ledger at v2.1.15 (Features moved to Ready for QA 🧪)
- `devlog.md` -> Devlog at v2.1.15
- `chatHandOff.md` -> Active handoff capsule at v2.1.15
...
2. Flight QA Pass:
   ...
   - Confirm Auto-Warp cruises around the Moon at 60x without stalling down to 10x.
   - On inbound Earth return cruise, verify that Auto-Warp automatically downshifts to 300x Green, then 60x Green at 20,000 km.
   - During re-entry, verify smooth coasting without the ~10s spurious bank-up correction between 65 km and 35 km.
   - Below 35 km during parachute descent, verify 10x Yellow advisory warp is available and functions smoothly, dropping automatically to 1x for 30 seconds before Drogues, Mains, and Splashdown.
   - Upon splashdown, verify flight log JSON includes `sim_flight_time` and `sim_flight_seconds`.
