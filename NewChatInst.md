...
## 🧠 Permanent System Memory & Context Protocols
*(Always preserve these rules across sessions)*
1. **The Privacy & Network Boundary**: Always treat the `/private` directory as a strictly isolated repository. Any references to specific high-profile individuals (e.g., Hubble VIPs) or local aerospace reconnaissance (Aerojet/L3Harris) must absolutely not merge into the public `devlog.md`, `README.md`, or the public `lunar-flyby-xr` GitHub repository.
2. **Mobile Design Protocol**: The mobile application is strictly locked to **Portrait Mode**. Do not attempt to force CSS landscape responsiveness. The UI requires a targeted, separate touch-friendly iteration explicitly designed to prevent "fat-finger" misclicks on dangerous time-warp thresholds.
3. **Three.js Camera Targeting Math**: The WebXR camera targeting requires subtracting the physics velocity vector from the position vector for the `lookAt` orientation (since Three.js maps the positive-Z axis forward, but cameras face negative-Z). 
4. **3D Scale & Geometry Clipping**: The Three.js simulation uses a `renderScale = 0.02` (1 coordinate unit = 50km). Because of this scale, fixed UI geometry (like the Prograde vector ring at `Z: -15`) will physically clip into the Earth mesh when dropping below 100km. Camera-relative floating HUD features must account for this scaling.
5. **Single-File Architecture Restriction**: Unless explicitly triggered in the V2.0 modularization task, the public simulation must remain a 100% self-contained, single-file architecture (`index.html`) without reliance on local bundlers or split scripts—maximizing immediate browser accessibility.
